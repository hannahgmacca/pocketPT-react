...
class AuthAPI {
  ...
  public async login(auth: { email: string; password: string }): Promise<AuthData> {
    return await this.client.post<AuthData>('/auth/signin', auth);
  }

  public async signUp(auth: { email: string; password: string }): Promise<AuthData> {
    return await this.client.post<AuthData>('/auth/signup', auth);
  }
